...
// parts of my program
std::vector<double> quartic_calculator(double a, double, b, double c, double d) {
...
double quartic_function(double lamda, conserved_variables P, double p_g, double rho)
{
	...
	double abs_b_squared = std::abs(B_squared) / (gamma * gamma) +
						   (P.B_x * P.v_x + P.B_y * P.v_y + P.B_z * P.v_z) * (P.B_x * P.v_x + P.B_y * P.v_y + P.B_z * P.v_z);

	double F_of_lamda = w_g * (1 - c_s_squared) * std::pow(a, 4.0) - (1 - lamda * lamda) * ((abs_b_squared + w_g * c_s_squared) * a * a - c_s_squared * special_B_squared);
	return F_of_lamda;
}
...
double bisection_loop_lamda(conserved_variables P, double left_lamda, double right_lamda, double p_g, double v_x, double rho)
{
	...
	bool above_value_triggered {false};
	bool below_value_triggered {false};
	double max_iterations = 20;
	double iterations = 0;


	while (std::abs(left_lamda - right_lamda) > error_margin && iterations < max_iterations)
	{
		iterations++;
		midpoint_lamda = (left_lamda + right_lamda) / 2.0;
		...
		if (f_midpoint * f_left < 0)
		{
			...
			below_value_triggered = true;
		}
		else if (f_midpoint * f_left > 0)
		{
			left_lamda = midpoint_lamda;
			above_value_triggered = true;
		}
		else
		{
	...
	if (!below_value_triggered || !above_value_triggered) {
		std::cout << "the bisection method did not converge, or lamda was close to -1,1 \n";
	}
	std::cout << ((left_lamda + right_lamda) / 2.0) << std::endl;
